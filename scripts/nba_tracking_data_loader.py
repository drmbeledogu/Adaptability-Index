# Modified from https://huggingface.co/datasets/dcayton/nba_tracking_data_15_16
# Please adhere to the license below
...
class NbaTracking(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        # Access the specific game name from config
        game_name = self.config.name

        # Check if the game exists in the ITEMS list
        matching_game = [item for item in self.items if item['name'] == game_name]
        if not matching_game:
            raise ValueError(f"Game '{game_name}' not found.")

        # Prepare the URL for the game
        _URLS = {matching_game[0]['name'][:-3]: _URL + "/" + matching_game[0]['name']}
        data_dir = dl_manager.download_and_extract(_URLS)
        # print([os.listdir(directory) for directory in data_dir.values()])

        all_file_paths = {
            key: os.path.join(directory, os.listdir(directory)[0]) for key, directory in data_dir.items()
        }
